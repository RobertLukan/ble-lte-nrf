...
static void cloud_event_handler(const struct nrf_cloud_evt *nrf_cloud_evt)
{
	...
	//	LOG_INF("%d bytes received from cloud", nrf_cloud_evt->data.len);
	//	LOG_INF("%d bytes received from cloud", nrf_cloud_evt->data.ptr);
		//nrf_cloud_data nrfCLoudData = nrf_cloud_evt->data.ptr;
	//	nrf_cloud_data  destination; 
		char grx_buf[1000];

		for(int i = 0; i < nrf_cloud_evt->data.len; i++) 
		{
				memcpy(&grx_buf[i], (uint8_t*) nrf_cloud_evt->data.ptr+i, 1);

		}
//		LOG_INF("%s bytes received from cloud",grx_buf);
		cJSON *json = cJSON_Parse(grx_buf);
		const cJSON *cGpio = NULL;

    	cGpio = cJSON_GetObjectItemCaseSensitive(json, "appId");
    	if (cJSON_IsString(cGpio) && (cGpio->valuestring != NULL))
    	{

			if(!strcmp(cGpio->valuestring,"REBOOT"))
			{
				LOG_INF("Mathched Reboot\"%s\"\n", cGpio->valuestring);
				LOG_WRN("Rebooting...");
				LOG_PANIC();
